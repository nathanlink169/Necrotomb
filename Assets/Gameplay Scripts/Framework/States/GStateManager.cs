    ...
    public class GStateManager : SingletonBehaviour<GStateManager>
    {
        ...
        void Update()
        {
            if (!m_bLoading && m_loadingScreen != null)
            {
                ...
                {
                    if (subStateToExit != null)
                    {
                        // exit sub state
                        // and resume the next one in the queue
                        UnloadSubState(subStateToExit);
                        ResumeSubState();
                    }
                    else if (!ReferenceEquals(nextSubStateId, UNDEFINED_STATE) &&
...
#endif
            if (ReferenceEquals(in_nextState, m_currentState.StateId))
            {
                return toChange;
            }
        ...
        public BaseState FindSubState(string stateId)
        {
            StateInfo si;
            for (int i = 0; i < m_gameSubStates.Count; ++i)
            {
        ...
        {
            // if in the editor we want to force the state info if there wasn't any before
#if UNITY_EDITOR || UNITY_EDITOR_OSX
            if (m_currentState == null)
            {
                ForceStateInfo(in_stateInfo);
            }
#endif
                ...
                in_stateInfo.StateId == m_sNextStateId)
            {
                ...
                m_sPreviousStateId = m_currentState.StateId;
        ...
        // in order to force the current state
        ...
        private bool UnloadSubState(StateInfo in_subStateToExit)
        {
            ...
            if (in_subStateToExit != null && in_subStateToExit.State != null)
            {
                ...
                // remove the current substate from the two arrays
                if (m_gameSubStates.Count > 0) m_gameSubStates.Remove(in_subStateToExit);
                if (m_gameSubStatesToPop.Count > 0) m_gameSubStatesToPop.Remove(in_subStateToExit);
        ...
        private string m_sPreviousStateId = GStateManager.UNDEFINED_STATE;
        private string m_sNextStateId = GStateManager.UNDEFINED_STATE;
        private string m_sPreviousSubStateId = GStateManager.UNDEFINED_STATE;
        private string m_sNextSubStateId = GStateManager.UNDEFINED_STATE;

        private string m_sNextStateFlowId = GStateManager.UNDEFINED_STATE;
